...
void main() {
    ...
    FLOAT16_4 color = FLOAT16_4(texelFetch(TEX_TRANSLUCENT_FINAL, uv, 0));
    ...
    float depthTranslucent = texelFetch(TEX_DEPTH_TRANS, uv, 0).r;
        ...
        FLOAT16_3 tintColor = FLOAT16_3(texelFetch(TEX_GB_COLOR, uv, 0).rgb);
        tintColor = RGBToLinear(tintColor);
    #endif

    FLOAT16_4 meta = FLOAT16_4(unpackUnorm4x8(texelFetch(TEX_GB_SPECULAR, uv, 0).g));


    #ifdef REFRACT_ENABLED
        FLOAT16_4 normalData = FLOAT16_4(texelFetch(TEX_GB_NORMALS, uv, 0));
        ...
        if (hasTrans) {
            const FLOAT16 eta = FLOAT16(1.0 / 1.33);
        ...
        FLOAT16_3 src = FLOAT16_3(texture(TEX_FINAL, coord).rgb);
    #else
        FLOAT16_3 src = FLOAT16_3(texelFetch(TEX_FINAL, uv, 0).rgb);
    #endif
    ...
    vec3 transViewPos = project(MAT_PROJ_INV, transNdcPos);
        ...
        uint matID = uint(meta.a * 255.0 + 0.5);
            ...
            if (matID == MAT_WATER && isEyeInWater != 1 && transViewPos.z > opaqueViewPos.z) {
                FLOAT16 waterDepth = FLOAT16(distance(opaqueViewPos, transViewPos));

                #ifdef WATER_ABSORPTION
                    FLOAT16_3 waterAbsorbColorL = FLOAT16(1.0) - normalize(tintColor);
                    src *= FLOAT16_3(GetWaterAbsorption(waterDepth, waterAbsorbColorL));
                #endif

                #ifdef WATER_FOG_FIX
                    FLOAT16 borderFogF = FLOAT16(GetBorderFogStrength(waterDepth));
                    FLOAT16 envFogF = FLOAT16(GetEnvFogStrength(waterDepth, true));

                    FLOAT16_3 fogColorFinal = FLOAT16_3(GetWaterFogColor(tintColor, sunLocalDir, weatherStrength, skyDayF));
...
//    color.rgb = mix(src, color.rgb, color.a);
    color.rgb += src * saturate(FLOAT16(1.0) - color.a);

    #if LIGHTING_MODE == LIGHTING_MODE_ENHANCED && (defined(WATER_FOG_FIX) || defined(WATER_ABSORPTION))
        if (isEyeInWater == 1) {
            FLOAT16 waterDepth = FLOAT16(length(transViewPos));

            #ifdef WATER_ABSORPTION
                FLOAT16_3 waterAbsorbColorL = FLOAT16(1.0) - normalize(RGBToLinear(FLOAT16_3(fogColor)));
                color.rgb *= FLOAT16_3(GetWaterAbsorption(waterDepth, waterAbsorbColorL));
            #endif
