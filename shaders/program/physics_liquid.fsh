...
//    #endif
    ...
    vec4 color = vec4(1.0, 0.0, 0.0, Water_OpacityF); // vec4(vec3(1.0), Water_OpacityF);
...
//                ApplyWetnessPuddles(texNormal, vIn.localPos, skyWetness, porosity, puddleF);
                ...
                if (weatherStrength > EPSILON)
                    ApplyWetnessRipples(localNormal, rippleNormalStrength);
            #endif
        #endif
    #endif
...
//    vec3 texViewNormal = mat3(gbufferModelView) * localNormal;
...
//            }
        ...
        vec3 specularFinal = vec3(0.0);
            ...
            GetFloodfillLighting(diffuseFinal, specularFinal, vIn.localPos, localNormal, localNormal, lmFinal, shadowColor, albedo, metal_f0, roughL, occlusion, sss, false);
...
//            diffuseFinal += emission * MaterialEmissionF;
            ...
            GetSkyLightingFinal(diffuseFinal, specularFinal, shadowColor, vIn.localPos, localNormal, localNormal, albedo, vIn.lmcoord, roughL, metal_f0, occlusion, sss, isUnderWater, tir);
        #else
            diffuseFinal += WorldAmbientF;
        #endif

        #if LIGHTING_MODE_HAND != HAND_LIGHT_NONE
            SampleHandLight(diffuseFinal, specularFinal, vIn.localPos, localNormal, localNormal, albedo, roughL, metal_f0, occlusion, sss);
        #endif
...
//        #endif
        ...
        #if MATERIAL_REFLECTIONS != REFLECT_NONE
//            if (isWater) {
                float skyNoVm = max(dot(localNormal, -localViewDir), 0.0);
