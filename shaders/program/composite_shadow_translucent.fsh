...
void main() {
        ...
        if (depthTransL < depthOpaqueL) {
            ...
            uvec2 deferredDataA = texelFetch(BUFFER_DEFERRED_DATA_A, uv, 0).rg;
            vec3 localNormal = unpackUnorm4x8(deferredDataA.r).rgb;
            // #if MATERIAL_SSS != 0
            ...
            #if MATERIAL_SSS != 0
                float sss = unpackUnorm4x8(deferredDataA.r).w;
