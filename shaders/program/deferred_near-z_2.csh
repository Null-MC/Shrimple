#define RENDER_DEFERRED_HI_Z_RAD_2
...
void main() {
    ...
    populateSharedBuffer(kernelPos, localPos, globalPos, 0);
    barrier();

	if (any(greaterThanEqual(globalPos, ivec2(ceil(viewSize * 0.25))))) return;

	float minZ = getSharedBufferMinZ(kernelPos);
	writeNearTileMinZ(kernelPos, globalPos, minZ, 1);
}
