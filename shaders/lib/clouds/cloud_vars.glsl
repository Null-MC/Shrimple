...
const vec3 CloudScatterColor_rain  = _RGBToLinear(vec3(0.9));

const vec3 CloudAbsorbColor_clear = _RGBToLinear(1.0 - vec3(0.718, 0.737, 0.769));
const vec3 CloudAbsorbColor_rain  = _RGBToLinear(1.0 - vec3(0.7));

float CloudDensityF       = mix(0.4, 0.6, skyRainStrength);
...
float CloudAbsorbF        = mix(0.08, 0.36, skyRainStrength);
