vec2 GetLinearCoords(const in vec2 texcoordFull, out ivec2 uv[4]) {
    vec2 f = fract(texcoordFull - 0.5);

//    ivec2 iuv[4];
    uv[0] = ivec2(texcoordFull - f - 0.5);
    uv[1] = uv[0]+ivec2(1, 0);
    uv[2] = uv[0]+ivec2(0, 1);
    uv[3] = uv[0]+ivec2(1, 1);
...
vec2 GetLinearCoords(const in vec2 texcoord, const in vec2 texSize, out ivec2 uv[4]) {
    vec2 texcoordFull = texcoord * texSize;
    return GetLinearCoords(texcoordFull, uv);

//    vec2 f = fract(texcoordFull);
//    vec2 pixelSize = 1.0 / texSize;

//    uv[0] = texcoord - f*pixelSize;
//    uv[1] = fract(uv[0] + vec2(1.0, 0.0)*pixelSize);
//    uv[2] = fract(uv[0] + vec2(0.0, 1.0)*pixelSize);
//    uv[3] = fract(uv[0] + vec2(1.0, 1.0)*pixelSize);
//
//    // TODO: wtf is this?
//    uv[0] = fract(uv[0]);
//
...
vec3 TextureLinearRGB(const in sampler2D samplerName, const in ivec2 uv[4], const in vec2 f) {
    vec3 samples[4];
    samples[0] = texelFetch(samplerName, uv[0], 0).rgb;
    samples[1] = texelFetch(samplerName, uv[1], 0).rgb;
    samples[2] = texelFetch(samplerName, uv[2], 0).rgb;
    samples[3] = texelFetch(samplerName, uv[3], 0).rgb;
    return LinearBlend4(samples, f);
}

vec3 TextureLinearRGB(const in sampler2D samplerName, const in vec2 texcoord, const in vec2 texSize) {
    ivec2 uv[4];
    vec2 f = GetLinearCoords(texcoord, texSize, uv);
    return TextureLinearRGB(samplerName, uv, f);
}

//vec3 TextureLinearBiasRGB(const in sampler2D samplerName, const in ivec2 uv[4], const in float bias, const in vec2 f) {
//    vec3 samples[4];
...
//float TextureLodLinear(const in sampler2D samplerName, const in ivec2 uv[4], const in float lod, const in vec2 f, const in int comp) {
//    vec4 samples;
...
//}
//
//float TextureGradLinear(const in sampler2D samplerName, const in ivec2 uv[4], const in mat2 dFdXY, const in vec2 f, const in int comp) {
//    vec4 samples;
...
//vec3 TextureLodLinearRGB(const in sampler2D samplerName, const in ivec2 uv[4], const in int lod, const in vec2 f) {
//    vec3 samples[4];
...
//}
//
//vec3 TextureGradLinearRGB(const in sampler2D samplerName, const in ivec2 uv[4], const in mat2 dFdXY, const in vec2 f) {
//    vec3 samples[4];
