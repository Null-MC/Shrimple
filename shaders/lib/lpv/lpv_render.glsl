...
vec3 GetLpvSamplePos(const in vec3 lpvPos, const in vec3 geoNormal, const in vec3 texNormal, const in float offset) {
    #if MATERIAL_NORMALS != 0
        vec3 minPos = floor(lpvPos + offset * geoNormal);

        vec3 offsetPos = lpvPos + offset * texNormal;
...
vec4 SampleLpv(const in vec3 lpvPos, const in vec3 geoNormal, const in vec3 texNormal) {
    vec3 samplePos = GetLpvSamplePos(lpvPos, geoNormal, texNormal);
