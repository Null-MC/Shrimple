...
//                if (tir) skyReflectF = vec3(1.0);
                ...
                //accumDiffuse *= 1.0 - skyReflectF;
                    ...
                    reflectLocalPos += localNormal * 0.05;
                    //vec3 reflectViewPos = viewPosOpaque;

//                    #ifdef MATERIAL_REFLECT_PIXELATE
//                        vec3 cameraPosFract = fract(cameraPosition);
//                        vec3 worldPos = reflectLocalPos + cameraPosFract;
//                        worldPos += localNormal * rcp(MATERIAL_RESOLUTION);
//
//                        vec3 f = floor(fract(worldPos) * MATERIAL_RESOLUTION + EPSILON) + 0.5;
//                        reflectLocalPos = floor(worldPos) - cameraPosFract + f / MATERIAL_RESOLUTION;
//
//
