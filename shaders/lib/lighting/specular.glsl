#ifndef __INC_LIGHTING_SPECULAR
#define __INC_LIGHTING_SPECULAR
...
vec3 SampleLightSpecular(vec3 F, vec3 normal, vec3 viewDir, vec3 H, const in float NoLm, float roughL) {
    float NoH = max(dot(normal, H), 0.0);
    float NoV = max(dot(normal, viewDir), 0.0);

    float alpha = max(roughL, 0.006);
    return NoLm * D_GGX(NoH, alpha) * V_Approx(NoLm, NoV, alpha) * F;
}
...
vec3 SampleLightSpecular(vec3 albedo, vec3 normal, vec3 lightDir, vec3 viewDir, const in float NoLm, float roughL, float specularG) {
    ...
    return SampleLightSpecular(F, normal, viewDir, H, NoLm, roughL);
}
