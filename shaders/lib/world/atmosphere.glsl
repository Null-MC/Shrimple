...
const float FogDensity_Cave = SKY_CAVE_FOG_DENSITY * 0.01;
    ...
    const float AirAmbientF = 0.1;//mix(0.02, 0.0, weatherStrength);
    vec3 AirScatterColor = vec3(mix(0.09, 0.05, weatherStrength));
    // const vec3 AirExtinctColor = _RGBToLinear(1.0 - vec3(0.6));//mix(0.02, 0.006, weatherStrength);
    vec3 AirExtinctColor = vec3(mix(0.08, 0.06, weatherStrength));//mix(0.02, 0.006, weatherStrength);
#else
